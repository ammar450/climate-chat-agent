...
def generate_markdown_report(all_runs: List[Dict], aggregate: Dict, seed: int, model: str, output_path: str):
    ...
    L.append(f"| Success Rate - Query Execution (LLM Estimate) | {m.get('avg_execution_success_rate', 0):.1%} |")
...
def main():
    ...
    print(f"  Success - Query Execution (LLM Estimate):  {m['avg_execution_success_rate']:.1%}")
